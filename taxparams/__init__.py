    ...
    def adjust(self, params_or_path, **kwargs):
        ...
        if params.get("CPI_offset") is not None:
            ...
            for param in self._data:
                if self._data[param].get("indexed", False):
                    print("re-indexing param", param, cpi_min_year)
        ...
        for param, values in params.items():   
            if param.endswith("-indexed"):
                ...
                self._adjust({base_param: [{"value": None}]})
                self._adjust({base_param: self._init_values[base_param]})
                ...
                reset_state = reset_state | {base_param}
